...
1. **Native (Go) actions** — implement `action.Action` and `Registry.Register`
   ...
   `service_status`, `log`, `hostname`, `timezone`) are exactly these.
...
| `action` | `Action` interface + registry + builtins (`run`, `package`, `file`, `file_line`, `service`, `service_status`, `log`, `hostname`, `timezone`) |
